...
const Render = ({articlesFetch, superTags}) => {
    ...
    // tags filtered by articles text
    ...
    // #Step 3 (Tags): Adds tag to articles tags
    ...
    // #Step 2 (Tags): Removes tag from tags, and filterTags based on id, pushes to articles tags
    function removeTag(id) {
        // find the tag to remove
        const item = tags.find((tag) => {
            return tag.id === id;
        });
        // push it to end of articles tags
        pushSearchTag(item);
    ...
    // removes all the tags selected by the user in the articles bar
    function clearAllSearchTags() {

        // add the articles tags to all tags and filter tags
        setFilterTags(prev => {
